---
title: RabbitMQ 跨网络 MQTT 双向通信配置
date: 2025-07-22
tag:
  - RabbitMQ
  - MQTT
  - Federation
  - 中间件
description: 通过 RabbitMQ Federation 插件实现内网与外网 RabbitMQ 之间的双向 MQTT 消息转发，支持业务消息下发到外网设备、外网设备上报回传内网业务。包含参数详解、清理回退、验证脚本。
sticky: 1
---

# RabbitMQ 跨网络 MQTT 双向通信配置文档

> **拓扑：** `业务系统 → 内网RabbitMQ(133) ←[Federation]→ 外网RabbitMQ(134) → 外网设备`
>
...
1. [环境信息](#1-环境信息)
2. [基础配置（Docker 部署 RabbitMQ）](#2-基础配置docker-部署-rabbitmq)
3. [启用核心插件](#3-启用核心插件)
...
| 项目 | 值 |
|------|-----|
| 内网 RabbitMQ IP | `192.168.6.133` |
| 外网 RabbitMQ IP | `192.168.6.134` |
| 操作系统 | 虚拟机 CentOS / Ubuntu（Docker 部署） |
| RabbitMQ 版本 | `3.13.7`（`my-rabbitmq:3-management` 镜像） |
| 管理端口 | 15672（HTTP API） |
| MQTT 端口 | 1883 |
| AMQP 端口 | 5672 |
| 管理账号 | `admin` / `123456` |
...
> - 如果使用不同镜像名，请将 `my-rabbitmq:3-management` 替换为实际镜像名

### 2.2 确认容器运行

```bash
docker ps --format '{{.Names}} {{.Image}} {{.Ports}}' | grep rabbit
```

预期输出：
```
rabbitmq my-rabbitmq:3-management 0.0.0.0:1883->1883/tcp, 0.0.0.0:5672->5672/tcp, 0.0.0.0:15672->15672/tcp
```

### 2.3 验证管理界面可访问

在浏览器访问：
- 内网：`http://192.168.6.133:15672`（账号 `admin` / `123456`）
- 外网：`http://192.168.6.134:15672`（账号 `admin` / `123456`）
...
```bash
# 获取容器 ID
CONTAINER_ID=$(docker ps -q --filter name=rabbit)
echo "容器ID: $CONTAINER_ID"

# 启用插件（一次性全部启用）
docker exec $CONTAINER_ID rabbitmq-plugins enable \
  rabbitmq_mqtt \
...
```bash
docker exec $CONTAINER_ID rabbitmq-plugins list -e
```

预期输出应包含以下 `[E*]` 标记的插件：
```
[E*] rabbitmq_mqtt                     3.13.7
[E*] rabbitmq_shovel                   3.13.7
[E*] rabbitmq_shovel_management        3.13.7
[E*] rabbitmq_federation               3.13.7
[E*] rabbitmq_federation_management    3.13.7
...
```bash
curl -s -u admin:123456 http://192.168.6.133:15672/api/nodes/rabbit@$(docker ps -q --filter name=rabbit) \
  | python3 -c "import sys,json;print(json.load(sys.stdin).get('enabled_plugins'))"
```
...
| 参数 | 必填 | 类型 | 默认值 | 说明 |
|------|------|------|--------|------|
| `uri` | **必填** | 字符串 | - | 上游 RabbitMQ 的连接地址。格式：`amqp://用户名:密码@IP:5672`。示例：`amqp://admin:123456@192.168.6.134:5672` |
...
| `expires` | 可选 | 整数 | `never`（不过期） | Federation 链接的空闲过期时间（毫秒）。超过该时间没有消息流动则自动断开。通常不设 |
| `message-ttl` | 可选 | 整数 | `never`（不过期） | 消息在 Federation 队列中的最大存活时间（毫秒）。超过则丢弃。通常不设 |
| `queue-type` | 可选 | 字符串 | `classic` | Federation 内部使用的队列类型。可选 `classic` 或 `quorum`。`quorum` 更可靠但更慢 |
| `ha-policy` | 可选 | 字符串 | - | 高可用策略。单机部署不需要 |
| `set-priority` | 可选 | 整数 | - | 设置 Federation 消费队列的优先级 |

**常见 uri 格式：**

- 明文：`amqp://admin:123456@192.168.6.134:5672`
- 带虚拟主机：`amqp://admin:123456@192.168.6.134:5672/my-vhost`
- SSL：`amqps://admin:123456@192.168.6.134:5671`
- 多个地址：`["amqp://node1","amqp://node2"]`
...
> **💡 提示：** Policy 创建后 **立即生效**，无需重启 RabbitMQ。
...
```bash
curl -s -u admin:123456 -X DELETE \
...
1. 访问 `http://<IP>:15672` → 用 `admin` / `123456` 登录
...
> **⚠️ 注意顺序：** 必须先删除 Policy 再删除 Upstream。如果先删 Upstream，Policy 引用了不存在的 Upstream 会导致 Federation 报错，但 Policy 本身仍然可删除。
...
```bash
curl -s -u admin:123456 -X PUT \
  -H "Content-Type: application/json" \
  http://192.168.6.133:15672/api/parameters/federation-upstream/%2f/upstream_134 \
  -d '{
    "value": {
      "uri": "amqp://admin:123456@192.168.6.134:5672",
      "exchange": "amq.topic",
      "max-hops": 1,
      "prefetch-count": 1000,
      "reconnect-delay": 1,
...
curl -s -u admin:123456 -X PUT \
  ...
  -d '{
    "value": {
      "uri": "amqp://admin:123456@192.168.6.133:5672",
      "exchange": "amq.topic",
      "max-hops": 1,
      "prefetch-count": 1000,
      "reconnect-delay": 1,
...
> Federation 通过 Policy 来生效——Policy 告诉 RabbitMQ 哪些 Exchange 需要被联合，以及使用哪个 Upstream。
...
curl -s -u admin:123456 http://192.168.6.133:15672/api/federation-links | python3 -c \
  "import sys,json; [print(f'{l.get(\"exchange\",\"?\")}: {l.get(\"status\",\"?\")}') for l in json.load(sys.stdin)]"

# 查看外网 134 的 Federation 链接
curl -s -u admin:123456 http://192.168.6.134:15672/api/federation-links | python3 -c \
  "import sys,json; [print(f'{l.get(\"exchange\",\"?\")}: {l.get(\"status\",\"?\")}') for l in json.load(sys.stdin)]"
```
...
也可通过管理界面查看：访问 `http://任意节点IP:15672` → **Admin** → **Federation Status**
...
同目录下的 `verify_federation.py` 是独立的验证脚本，可直接上传到服务器执行：

```bash
# 1. 修改脚本中的 IP 配置（默认 192.168.6.133 / 192.168.6.134）
# 2. 执行
python verify_federation.py
```
...
curl -s -u admin:123456 -X PUT \
  -H "Content-Type: application/json" \
  http://192.168.6.133:15672/api/parameters/federation-upstream/%2f/upstream_134 \
  -d '{"value":{"uri":"amqp://admin:123456@192.168.6.134:5672","exchange":"amq.topic","max-hops":1,"prefetch-count":1000,"reconnect-delay":1,"ack-mode":"on-confirm","trust-user-id":false}}'
...
curl -s -u admin:123456 -X PUT \
  -H "Content-Type: application/json" \
  http://192.168.6.134:15672/api/parameters/federation-upstream/%2f/upstream_133 \
  -d '{"value":{"uri":"amqp://admin:123456@192.168.6.133:5672","exchange":"amq.topic","max-hops":1,"prefetch-count":1000,"reconnect-delay":1,"ack-mode":"on-confirm","trust-user-id":false}}'
...
1. **确认 Federation 链接状态**
   ```bash
   curl -s -u admin:123456 http://IP:15672/api/federation-links
   ```
   确保显示 `"status": "running"`

2. **确认插件已启用**
   ```bash
   docker exec $(docker ps -q --filter name=rabbit) rabbitmq-plugins list -e
   ```
   确保以下插件已启用：`rabbitmq_mqtt`、`rabbitmq_federation`、`rabbitmq_federation_management`
...
4. **确认 MQTT 客户端用了正确的账号密码**
   MQTT 连接需要同时设置用户名和密码（`admin` / `123456`），不要留空。

5. **测试 AMQP 直连是否正常**
   ```bash
   # 用 Python 测试 AMQP 连接
   python3 -c "import pika; c=pika.BlockingConnection(pika.URLParameters('amqp://admin:123456@192.168.6.134:5672')); c.close(); print('AMQP OK')"
   ```
...
1. 先删除 Policy（见 [4.3 节](#43-清理旧配置填错时使用)）
2. 再删除 Upstream
3. 重新按正确参数创建 Upstream + Policy

> 如果只改 Upstream 不涉及 Policy，可以只删除 Upstream 重新创建，Policy 会自动关联更新。

### Q6: 如何为不同的主题使用不同的转发策略？

如果需要只转发特定主题（而不是全部 `#`），可以：

1. **方案一：使用 Shovel 替代 Federation** — 在 Queue 级别指定绑定 key（适合单向）
2. **方案二：创建多个 Federation Upstream** — 每个 Upstream 使用不同的 `exchange` 和 binding key（需自定义 Exchange，不适合 `amq.topic`）

当前配置（`amq.topic` + `#`）转发所有主题，满足大多数场景。

---

> **文档版本：** v1.1 | **最后更新：** 2025-07-20
> **RabbitMQ 版本：** 3.13.7 | **部署方式：** Docker
> **相关文件：** `verify_federation.py`（同目录下的验证脚本）
